...
function drawLineChart(id, data, color) {
  ...
  // Calcular padding izquierdo según longitud del label del eje Y
  const yLabelSample = formatBigNum(Math.round(maxV));
  ctx.font = '10px -apple-system,sans-serif';
  const yLabelW = ctx.measureText(yLabelSample).width;
  ...
  // X labels — espaciado inteligente
  ctx.fillStyle = text; ctx.font = '10px -apple-system,sans-serif'; ctx.textAlign = 'center';
  const minGap = 36; // px mínimos entre etiquetas
  const maxLabels = Math.floor(cw / minGap);
...
function drawBarChart(id, data, color) {
  ...
  for (let i = 0; i <= ticks; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(pad.l, y); ctx.lineTo(pad.l + cw, y); ctx.stroke();
    ctx.fillStyle = text; ctx.font = '10px -apple-system,sans-serif'; ctx.textAlign = 'right';
    ctx.fillText(formatBigNum(Math.round(maxV * i / ticks)), pad.l - 5, y + 3.5);
  }
